...
def main(json_path='options/train_baseline.json'):
    ...
    for epoch in range(opt["train"]["epochs"]):  # keep running
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step == guide_dropout_start_iter:
                            if hasattr(train_loader.dataset, 'proba_without_rgb'):
                                train_loader.dataset.proba_without_rgb = final_proba_without_rgb
                                if opt['rank'] == 0:
                                    logger.info(f'Training stabilized. Activated Random Guide Dropout with probability: {final_proba_without_rgb} at step {current_step}')
            # -------------------------------
